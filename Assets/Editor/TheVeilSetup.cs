...
namespace TheVeil.Editor
{
    ...
    /// can reproduce.
    /// </summary>
    public static class TheVeilSetup
    {
        ...
        const string RendererPath = SettingsDir + "/TheVeilUniversalRenderer.asset";
        const string PipelinePath = SettingsDir + "/TheVeilUniversalRenderPipeline.asset";
        ...
        [MenuItem("The Veil/Set Up Project")]
        public static void SetupProject()
        {
            ...
            Debug.Log($"[The Veil] Setup complete. Pipeline: {pipeline.name}, scene: {ScenePath}");
        }
        ...
        /// </summary>
        [MenuItem("The Veil/Set Up Play Scene")]
        public static void SetUpPlayScene()
        {
            ...
            AssetDatabase.SaveAssets();
            Debug.Log($"[The Veil] Play scene ready at {PlayScenePath}. Open it and press Play.");
        }
        ...
        internal static VisualLibrary LoadModels()
        {
            var library = new VisualLibrary
            {
                ...
                // Run The Veil > Build Animator Controllers before Set Up Play Scene.
        ...
        static void ReportFactions(VisualLibrary library)
        {
            ...
            if (library.EnemyFaction == null)
                Debug.LogWarning("[The Veil] No faction material for the enemy — the bandits fall "
            ...
            if (library.PlayerFaction == null)
                Debug.LogWarning("[The Veil] No faction material for the escort — your troops keep "
                                ...
                                string animator = null, float yaw = 0f, bool borrowed = false)
        {
            ...
            if (controller == null)
                Debug.LogWarning($"[The Veil] No animator for {Path.GetFileName(path)} — run The Veil > Build Animator Controllers.");
        ...
        const string StillWaterPath = SyntyNaturePack + "/Materials/Water/Water_01.mat";

        /// <summary>The fen's river: the meadow pack's stream, which the decorator tints bog-brown.</summary>
        const string MarshRiverPath = RiverMaterialPath;
        ...
        static BiomeDecor LoadForestDecor()
        {
            ...
            return new BiomeDecor
            {
                ...
                // <b>No falling water yet, and the reason is measured.</b> The rivers of the
                // fifty built levels drop at most 2.6 m between one tile and the next, and
                // the pack's fall is a river plane with a lip on it: stood in a step that
                // small it lies down flat, and 4-1 came back with white plates all over the
                // grass. TerrainDecorator.PlaceFalls is written and waiting; what it needs
                // is a cliff for the water to come over, and that is the generator's job,
                // not the dresser's.
                // <b>No sheet of water: the river is its own fall.</b> The pack's fall is a
                ...
                Kit = new BuildingKit
                {
                    ...
                    // The four paints the pack ships its buildings in. See BuildingKit.Colourways.
                    Colourways = new[]
                    {
                        AssetDatabase.LoadAssetAtPath<Material>($"{SyntyKnightsPack}/Materials/PolyKnights_01.mat"),
                        AssetDatabase.LoadAssetAtPath<Material>($"{SyntyKnightsPack}/Materials/PolyKnights_02.mat"),
                        AssetDatabase.LoadAssetAtPath<Material>($"{SyntyKnightsPack}/Materials/PolyKnights_03.mat"),
                        AssetDatabase.LoadAssetAtPath<Material>($"{SyntyKnightsPack}/Materials/PolyKnights_04.mat")
                    },
        ...
        /// <summary>The meadow pack: the country this chapter is actually set in.</summary>
        const string MeadowDir = "Assets/Synty/PolygonNatureBiomes/PNB_Meadow_Forest/Prefabs";

        static PropSet Meadow(params string[] names) => new PropSet(false, Load(MeadowDir, names));
        ...
        ///
        /// <b>Dressed out of the meadow pack rather than out of the forest's.</b> The first
        /// version made the plains from the forest's own models repainted into the nature
        /// pack's autumn colourway, and from above it read as a wood that had turned - no
        /// flowers to speak of, grass in dark tufts, and stone that belonged under a canopy.
        /// The meadow pack has what the country is made of: wildflowers in patches, grass in
        /// three heights, meadow and fruit trees, and pale field stone. The old pack still
        /// carries the conifers at the treeline and the willows at the water, which it draws
        /// better.
        /// </summary>
        static BiomeDecor LoadPlainsDecor()
        {
            ...
            // The wood in the middle of a meadow: broad meadow trees and fruit trees, with a
            // few of the old pack's conifers for the dark verticals at the edge of it.
            decor.Pines = Mixed(
                Load(MeadowDir, new[]
                {
            ...
            decor.GroundCover = Meadow(
                ...
                "SM_Env_Grass_Med_Clump_01", "SM_Env_Grass_Med_Clump_02",
            ...
            decor.Bushes = Meadow("SM_Env_Bush_01", "SM_Env_Bush_02", "SM_Env_Bush_03",
                                  "SM_Env_Grass_Bush_01");

            // Field stone: loose in the grass, piled where it was cleared, and standing in
            // outcrops.
            decor.Rocks = Meadow("SM_Env_Rock_01", "SM_Env_Rock_02", "SM_Env_Rock_03",
                                 "SM_Env_Rock_04", "SM_Env_Rock_05", "SM_Env_Rock_06",
                                 "SM_Env_Rock_Small_01", "SM_Env_Rock_Small_Pile_01",
                                 "SM_Env_Rock_Small_Pile_02", "SM_Env_Rock_Ground_01",
                                 "SM_Env_Rock_Ground_02");

            // <b>Outcrops, not just piles.</b> The reference country has rock standing out
            // of the grass all over it - a five-metre face with grass growing over its
            // shoulder - and the pile models are ankle-high rubble by comparison. The
            // pack's own cliff pieces are what that is, so they stand in the field beside
            // the piles rather than only at the water's edge.
            decor.Boulders = Meadow("SM_Env_Rock_Cliff_01", "SM_Env_Rock_Cliff_02",
                                    "SM_Env_Rock_Cliff_03",
                                    "SM_Env_Rock_Cliff_01", "SM_Env_Rock_Cliff_02",
                                    "SM_Env_Rock_Cliff_03",
                                    "SM_Env_Rock_Pile_01", "SM_Env_Rock_Pile_02",
                                    "SM_Env_Rock_Pile_03", "SM_Env_Rock_Pile_04",
                                    "SM_Env_Rock_Pile_05", "SM_Env_Rock_Pile_06",
                                    "SM_Env_Rock_Pile_07", "SM_Env_Rock_Round_01",
                                    "SM_Env_Ground_Mound_Large_01", "SM_Env_Ground_Mound_Large_02");

            // The faces themselves: the cliff pieces, and the pack's own great slabs for
            // the ground that is already rock. See TerrainDecorator.PlaceFalls for the other
            // thing this set is used for - the lip a river comes over.
            decor.Cliffs = Meadow("SM_Env_Rock_Cliff_01", "SM_Env_Rock_Cliff_02",
                                  "SM_Env_Rock_Cliff_03", "SM_Env_Ground_Cliff_Large_01",
                                  "SM_Env_Ground_Cliff_Large_02");

            decor.Shore = Meadow("SM_Env_Rock_Pile_01", "SM_Env_Rock_Pile_03",
                                 "SM_Env_Rock_Small_Pile_01", "SM_Env_Rock_Small_Pile_02");

            decor.Horizon = PaleStone(Synty("Terrain", "SM_Terrain_Mountain_01",
                                            "SM_Terrain_Mountain_02", "SM_Terrain_Mountain_03"));

            // The track down the road, in the pack's own worn ground.
            decor.GroundPatches = Meadow("SM_Env_Ground_Cover_01", "SM_Env_Ground_Cover_02",
                                         "SM_Env_Ground_Cover_03");

            decor.Deadfall = Meadow("SM_Prop_Leaves_Branch_01", "SM_Prop_Leaves_Pile_01",
                                    "SM_Prop_Leaves_Pile_02");
            decor.DeadTrees = Synty("Trees", "SM_Tree_Dead_01", "SM_Tree_Stump_01",
                                    "SM_Tree_Stump_03");

            // A fence and a gate where the road passes somebody's ground.
            decor.Fences = MeadowProps("SM_Prop_Meadow_Fence_01", "SM_Prop_Meadow_Fence_02",
                                       "SM_Prop_Meadow_Fence_03", "SM_Prop_Meadow_Fence_04",
        ...
        /// <summary>
        /// The farmland: the meadow pack's worked country - crops, stone walls, fences,
        /// gates and a windmill, which no pack in this project had before.
        /// </summary>
        static BiomeDecor LoadFarmlandDecor()
        {
            var decor = LoadPlainsDecor();

            // Standing crop, which is the whole difference between a meadow and a field.
            ...
            decor.Trees = Meadow("SM_Env_Tree_Fruit_01", "SM_Env_Tree_Fruit_02",
                                 "SM_Env_Tree_Fruit_03", "SM_Env_Tree_Meadow_01",
                                 "SM_Env_Tree_Meadow_02");
            decor.Pines = Meadow("SM_Env_Tree_Meadow_01", "SM_Env_Tree_Meadow_02",
                                 "SM_Env_Tree_Fruit_01", "SM_Env_Tree_Birch_02");

            // Walls and fences, twice over: it is what this country is divided by.
            decor.Fences = Mixed(
                Load($"{MeadowDir}/Props", new[]
                {
                    "SM_Prop_Meadow_Fence_01", "SM_Prop_Meadow_Fence_02", "SM_Prop_Meadow_Fence_03",
                    "SM_Prop_Meadow_Fence_04", "SM_Prop_Meadow_Fence_05", "SM_Prop_Meadow_Fence_06",
                    "SM_Prop_Meadow_Fence_07", "SM_Prop_Meadow_Fence_Gate_01",
                    "SM_Prop_Meadow_Fence_Gate_02", "SM_Prop_Meadow_Fence_Railing_01"
                }),
                Load(MeadowDir, new[]
                {
                    "SM_Prop_Stonewall_Long_01", "SM_Prop_Stonewall_Small_01",
                    "SM_Prop_Stonewall_Small_02", "SM_Prop_Stonewall_End_01",
        ...
        static void FitWaterAndWinter(LevelRunner runner)
        {
            ...
            runner.Looks = new[]
            {
                ...
                new BiomeLook
                {
                    ...
                    // <b>The fen's river runs; the fen's pools stand.</b> Both were the same
                    // brown sheet, on the reasoning that a bog with a clear blue river
                    // through it is two countries - which is right about the colour and
                ...
                new BiomeLook
                {
                    ...
                    // Full thickness, not the three quarters it was. The thinning was aimed
                    // at the treeline and it took the grass with it, and the grass is what a
                    // player stands in. The trees are thinned where they are chosen instead.
                    Density = 1f,
                    Weather = One($"{SyntyNatureDir}/FX/FX_Leaves_Orange_01.prefab"),
                    // The lightest haze in the game. At 0.0035 the far half of a level came
                    // back washed out from above, which is a country nobody can see.
                    Fog = true,
                    FogColor = new Color(0.86f, 0.89f, 0.84f),
                    FogDensity = 0.0012f,
        ...
        const string SyntyKnightsPack = "Assets/Synty/PolygonKnights";

        const string SyntyKnightsDir = SyntyKnightsPack + "/Prefabs";
        ...
        static GameObject[] Load(string folder, string[] names)
        {
            ...
            foreach (var name in names)
            {
                ...
                else Debug.LogWarning($"[The Veil] Scenery model not found: {folder}/{name} (.prefab or .fbx)");
            }
        ...
        static UniversalRenderPipelineAsset EnsureRenderPipeline()
        {
            ...
            if (existing == null)
            {
                ...
                existing.name = "TheVeilUniversalRenderPipeline";
        ...
        /// takes the whole <c>TheVeil</c> menu off the menu bar, and a fresh clone then has
        ...
        /// assembly did not compile, and nothing under TheVeil is running at all.
        /// </summary>
        [InitializeOnLoad]
        static class MenuBootstrap
        {
            static MenuBootstrap()
            {
                ...
                EditorApplication.delayCall += () =>
                {
                    ...
                    {
                        Debug.Log("[The Veil] No menu scene in the project — making one now.");
                    ...
                    Debug.Log("[The Veil] Build scene list was wrong — the app would not have "
        ...
        public static void SetUpMenuScene()
        {
            ...
            Debug.Log($"[The Veil] Menu ready at {MenuScenePath}. Open it and press Play.");
        }
        ...
        static void BuildMenuScene()
        {
            ...
            var shell = new GameObject("Menu", typeof(TheVeil.UI.MenuShell));
            ...
            if (saved)
                Debug.Log($"[The Veil] Menu scene written to {MenuScenePath}. It is in the "
                          + "Project window under Assets/_Project/Scenes — double-click it "
                          + "and press Play.");
            else
                Debug.LogError($"[The Veil] Could not write the menu scene to {MenuScenePath}.");
        }
        ...
        static Material EnsureRouteMaterial()
        {
            ...
            var shader = Shader.Find("TheVeil/RouteOverlay");
            if (shader == null)
                throw new InvalidOperationException("Shader 'TheVeil/RouteOverlay' not found.");
        ...
        static Material EnsureGroundMaterial()
        {
            ...
            if (material == null)
            {
                var shader = Shader.Find("TheVeil/TerrainGround");
                if (shader == null)
                    throw new InvalidOperationException("Shader 'TheVeil/TerrainGround' not found.");
            ...
            else Debug.LogWarning("[The Veil] Ground detail texture missing; the ground will be flat colour.");
        ...
        static Material EnsureMaterial()
        {
            ...
            var shader = Shader.Find("TheVeil/TerrainVertexColor");
            if (shader == null)
                throw new InvalidOperationException("Shader 'TheVeil/TerrainVertexColor' not found.");
        ...
        /// opening the editor: -executeMethod TheVeil.Editor.TheVeilSetup.CaptureLevelPreview
        /// -veilOutput &lt;path&gt; [-veilChapter N] [-veilLevel N]
        /// </summary>
        public static void CaptureLevelPreview()
        {
            string output = ArgValue("-veilOutput") ?? "Logs/level-preview.png";
            int chapter = int.TryParse(ArgValue("-veilChapter"), out var c) ? c : 1;
            int level = int.TryParse(ArgValue("-veilLevel"), out var l) ? l : 1;
            int width = int.TryParse(ArgValue("-veilWidth"), out var w) ? w : 1280;
            int height = int.TryParse(ArgValue("-veilHeight"), out var h) ? h : 720;
            ...
            try
            {
                ...
                Debug.Log($"[The Veil] Captured {chapter}-{level} (seed {preview.Seed}, " +
        ...
        ///
        /// -executeMethod TheVeil.Editor.TheVeilSetup.CapturePlayScene -veilOutput &lt;path&gt;
        /// [-veilChapter N] [-veilLevel N] [-veilSteps N]
        ///
        ...
        {
            string output = ArgValue("-veilOutput") ?? "Logs/play.png";
            int chapter = int.TryParse(ArgValue("-veilChapter"), out var c) ? c : 1;
            int level = int.TryParse(ArgValue("-veilLevel"), out var l) ? l : 1;
            int steps = int.TryParse(ArgValue("-veilSteps"), out var s) ? s : 0;
            int width = int.TryParse(ArgValue("-veilWidth"), out var w) ? w : 1400;
            int height = int.TryParse(ArgValue("-veilHeight"), out var h) ? h : 1000;
            ...
            if (float.TryParse(ArgValue("-veilCamDistance"), out float distance)) runner.FollowDistance = distance;
            if (float.TryParse(ArgValue("-veilCamHeight"), out float camHeight)) runner.FollowHeight = camHeight;
            runner.Restart();
            ...
            // -veilLitGround swaps the ground onto Unity's own Lit shader. It settles
            ...
            if (ArgValue("-veilLitGround") != null)
            {
                ...
                Debug.Log("[The Veil] Ground swapped to stock URP Lit for this capture.");
            }
            ...
            {
                if (!float.TryParse(ArgValue("-veilDebugShadow"), out float debugShadow)) debugShadow = 0f;
                ...
                if (debugShadow > 0f)
                    Debug.Log($"[The Veil] Ground material {ground.name} drawing debug mode {debugShadow}.");
            }
            ...
            try
            {
                ...
                var run = runner.Run;
                Debug.Log($"[The Veil] Captured {chapter}-{level} after {steps} steps: " +
        ...
        /// -executeMethod TheVeil.Editor.TheVeilSetup.CaptureCharacters -veilOutput &lt;path&gt;
        ///
        ...
        {
            string output = ArgValue("-veilOutput") ?? "Logs/characters.png";
            ...
            int width = int.TryParse(ArgValue("-veilWidth"), out var w) ? w : 1800;
            int height = int.TryParse(ArgValue("-veilHeight"), out var h) ? h : 700;
            ...
            // -veilNoGround takes the floor away. A model standing too low and a model
            ...
            ground.SetActive(ArgValue("-veilNoGround") == null);
            ...
            // -veilOnly narrows the line-up to whoever matches, so one model can be
            // looked at close instead of at one seventh of the frame. -veilBindPose
            // leaves the animators alone, which is how a pose that comes from the clip
            // is told apart from one that comes from the model.
            string only = ArgValue("-veilOnly");
            ...
            // -veilBindPose spawns the models with no animator at all, so what shows
            ...
            bool still = ArgValue("-veilBindPose") != null;
            ...
            // -veilOnly, a distance chosen from the width alone stands two metres away
            ...
            try
            {
                ...
                Debug.Log($"[The Veil] Captured {troops.Length + enemies.Length} characters " +
        ...
        /// </summary>
        /// <summary>
        /// Measures every model in a folder: -veilModelDir &lt;path under Assets&gt;.
        ///
        ...
        /// </summary>
        [MenuItem("The Veil/Report Folder Dimensions")]
        public static void ReportFolderDimensions()
            => ReportDimensionsOf(ArgValue("-veilModelDir") ?? SyntyNatureDir);
        ...
        /// </summary>
        [MenuItem("The Veil/Report Selected Folder Dimensions")]
        public static void ReportSelectedFolderDimensions()
        {
            ...
            if (folders.Count == 0)
            {
                Debug.LogWarning("[The Veil] Select one or more folders in the Project window first.");
                return;
            }
        ...
        static void ReportDimensionsOf(string folder)
        {
            ...
            Debug.Log($"[The Veil] {folder}: {guids.Length} models and prefabs");
            ...
            foreach (var guid in guids)
            {
                ...
                Debug.Log($"[The Veil]   {Path.GetFileNameWithoutExtension(path)}: " +
        ...
        /// </summary>
        [MenuItem("The Veil/Report Building Kit")]
        public static void ReportBuildingKit()
        {
            ...
            {
                Debug.LogWarning("[The Veil] No building kit is wired.");
                return;
            }
        ...
        static void Measure(string what, PropSet set)
        {
            if (set == null || !set.Any)
            {
                Debug.Log($"[The Veil] {what}: none wired.");
                return;
            }
            ...
            Debug.Log($"[The Veil] {what}: {string.Join(", ", lines)}");
        }
        ...
        static void Assembled(string what, GameObject built)
        {
            if (built == null)
            {
                Debug.LogWarning($"[The Veil] the kit could not build a {what}.");
                return;
            }
            ...
            Debug.Log($"[The Veil] A {what} came out {bounds.size.x:0.0}×{bounds.size.z:0.0} "
                      + $"× {bounds.size.y:0.0} m from {pieces.Count} pieces: "
                      + string.Join(", ", pieces));
        }

        [MenuItem("The Veil/Report Model Dimensions")]
        public static void ReportModelDimensions()
        {
            ...
            {
                var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(path);
                if (prefab == null) { Debug.LogWarning($"[The Veil] missing {path}"); continue; }
                ...
                var bounds = ModelScaling.Measure(instance);
                Debug.Log($"[The Veil] {Path.GetFileNameWithoutExtension(path),-22} " +
        ...
        /// </summary>
        [MenuItem("The Veil/Report Actor Fit")]
        public static void ReportActorFit()
        {
            ...
            foreach (var (name, actor) in cast)
            {
                ...
                Debug.Log($"[The Veil] {name,-20} {box.size.x,5:F2} wide x {box.size.y,5:F2} tall   " +
                ...
                foreach (var renderer in actor.GetComponentsInChildren<Renderer>())
                    Debug.Log($"[The Veil]     {name}/{renderer.name,-24} " +
        ...
        /// </summary>
        [MenuItem("The Veil/Report Animation Clips")]
        public static void ReportAnimationClips()
        {
            ...
            foreach (var path in models)
            {
                ...
                Debug.Log($"[The Veil] {Path.GetFileNameWithoutExtension(path)}  rig={rig}  " +
        ...
        /// </summary>
        [MenuItem("The Veil/Report Rig Bones")]
        public static void ReportRigBones()
        {
            ...
            foreach (var path in models)
            {
                ...
                Debug.Log($"[The Veil] {Path.GetFileNameWithoutExtension(path)}: {names.Count} transforms");
                Debug.Log($"[The Veil]   {string.Join(" | ", names)}");
        ...
        /// </summary>
        [MenuItem("The Veil/Colour Untextured Materials")]
        public static void ColourUntexturedMaterials()
        {
            ...
            foreach (var (path, colour) in colours)
            {
                ...
                if (material == null) { Debug.LogWarning($"[The Veil] Material not found: {path}"); continue; }

                if (material.HasProperty("_BaseMap") && material.GetTexture("_BaseMap") != null)
                {
                    Debug.Log($"[The Veil] {Path.GetFileNameWithoutExtension(path)} has its texture back; left alone.");
                    continue;
                }
            ...
            AssetDatabase.SaveAssets();
            Debug.Log($"[The Veil] Coloured {painted} materials that have no texture to draw.");
        }

        /// <summary>
        /// Prints which texture every material in a folder ended up with:
        /// -veilModelDir &lt;path under Assets&gt;.
        ///
        ...
        /// </summary>
        [MenuItem("The Veil/Report Material Textures")]
        public static void ReportMaterialTextures()
        {
            ...
            string folder = ArgValue("-veilModelDir") ?? SyntyNaturePack;

            var guids = AssetDatabase.FindAssets("t:Material", new[] { folder });
            int missing = 0;

            Debug.Log($"[The Veil] {folder}: {guids.Length} materials");
            ...
            Debug.Log($"[The Veil] {missing} of {guids.Length} materials have no base map.");
        }

        [MenuItem("The Veil/Report Materials")]
        public static void ReportMaterials()
        {
            ...
            {
                var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(path);
                if (prefab == null) { Debug.Log($"[The Veil] missing {path}"); continue; }
                ...
                foreach (var renderer in prefab.GetComponentsInChildren<Renderer>())
                {
                    ...
                    foreach (var material in renderer.sharedMaterials)
                    {
                        if (material == null) { Debug.Log("[The Veil]   (null material)"); continue; }
                        ...
                        Debug.Log($"[The Veil] {Path.GetFileNameWithoutExtension(path)} :: {material.name} " +
        ...
        /// rebuilding the scene: `The Veil > Refresh Scene Assets`.
        ///
        ...
        [MenuItem("The Veil/Refresh Scene Assets")]
        public static void RefreshSceneAssets()
        {
            ...
            if (System.IO.File.Exists(MenuScenePath))
                Debug.Log($"[The Veil] Menu scene already at {MenuScenePath}.");
            else
                BuildMenuScene();
        ...
        static void RefreshOpenScene()
        {
            ...
            {
                Debug.Log("[The Veil] The eagle's materials carry no textures — wiring them now.");
                WireLooseTextures();
            }
                         ...
                         FindObjectsSortMode.None))
            {
                ...
                Debug.Log($"[The Veil] {runner.name}: wagons at {VisualLibrary.WagonHeight:0.0} m — "
                         ...
                         FindObjectsSortMode.None))
            {
                ...
                Debug.Log($"[The Veil] {preview.name}: marsh plants {Names(preview.Decor.MarshPlants)}. "
            ...
            if (touched == 0)
            {
                Debug.LogWarning("[The Veil] No LevelRunner or LevelPreview in the open scene. "
            ...
            Debug.Log($"[The Veil] Refreshed {touched} component(s) in {scene.name} "
        ...
        /// to the model: `The Veil > Wire Loose Textures`, `-veilModelDir &lt;path&gt;`.
        ///
        ...
        /// </summary>
        [MenuItem("The Veil/Wire Loose Textures")]
        public static void WireLooseTextures()
        {
            string folder = ArgValue("-veilModelDir") ?? "Assets/ThirdParty/Eagle";
            ...
            if (textures.Count == 0)
            {
                Debug.LogWarning($"[The Veil] No textures in {folder}. Nothing to wire.");
                return;
            }
            ...
            if (albedo == null)
            {
                Debug.LogWarning($"[The Veil] No albedo texture found in {folder}. "
            ...
            Debug.Log($"[The Veil] {material.name}: albedo={albedo.name} "
        ...
        /// </summary>
        [MenuItem("The Veil/Restyle Model Materials")]
        public static void RestyleModelMaterials()
        {
            ...
            if (repaired > 0) Debug.Log($"[The Veil] Cleared broken material remaps on {repaired} models.");
            ...
            try
            {
                ...
                foreach (var modelPath in modelPaths)
                {
                    ...
                    foreach (var sub in AssetDatabase.LoadAllAssetsAtPath(modelPath))
                    {
                        ...
                        else Debug.LogWarning($"[The Veil] {model}/{embedded.name}: {error}");
                    }
                }
            }
            finally
            ...
            AssetDatabase.SaveAssets();
            Debug.Log($"[The Veil] Restyled materials: {textured} models had textures extracted, " +
        ...
        static void ReportShadowState(Material ground)
        {
            if (ground != null)
            {
                ...
                Debug.Log($"[The Veil] ground material: {ground.name} shader={ground.shader.name} {values}");
            }
            ...
            Debug.Log($"[The Veil] shadow chain: pipeline={(pipeline == null ? "none" : pipeline.name)} " +
