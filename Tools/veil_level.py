"""A port of TheVeil.Sim and TheVeil.Gen to Python, so a level can be generated without Unity.

The engine is a Windows GUI binary and the capture methods in TheVeil.Editor.TheVeilSetup
...
Two things needed care, and both are load-bearing rather than pedantry:
  ...
  * Scenery models are counted, not loaded. The decorator's random stream advances by
    ...
    from LoadForestDecor in TheVeilSetup.cs.
"""
...
# TheVeil.View.TerrainDecorator: a quarter either way for rocks, grass and buildings, and
...
# TheVeil.View.TerrainDecorator for why each number is what it is.
...
# Model counts from LoadForestDecor in Assets/Editor/TheVeilSetup.cs.
SET_SIZES = {
...
#
# Mirrors TheVeil.Sim.Wildlife. Deer, foxes and boar graze over the level and scatter when
...
# Chansen att en skogsruta accepteras alls. Se TheVeil.Sim.Wildlife.ForestShare: att räkna
...
def _pick(terrain: int, rng: DeterministicRandom):
    """Mirrors TheVeil.View.TerrainDecorator.Pick — a weighted draw, shares in a column.
