...
The engine's own captures — TheVail.Editor.TheVailSetup.CaptureLevelPreview and
...
Tools/vail_level.py generates from a seed.
...
What is faithful, and what is not:
  ...
  * The camera, the sun and the ground shading are. Angles, field of view, orthographic
    ...
    the linear fog are all read off TheVailSetup.cs and TerrainGround.shader.
...
def _parse_waypoints(text: str, grid: A.TileGrid):
    ...
    for token in text.split():
        ...
        if not A.can_place_waypoint(grid, tile, placed):
            raise SystemExit(f"[The Vail] {token} is not somewhere a waypoint can go — "
...
def _print_preview(level: A.LevelMap, route, waypoints) -> None:
    ...
    if not route.valid:
        print(f"[The Vail] route: leg {route.failed_leg} has no path — the run cannot start")
        return
    ...
    print(f"[The Vail] route: {len(route.tiles)} tiles, cost {route.travel_cost:.1f}, "
          f"about {route.estimated_seconds():.0f} s")
    print(f"[The Vail] ground: {mix}")
    print(f"[The Vail] ambush weight {route.ambush_exposure:.2f} "
    ...
    if route.crossings:
        ...
        print(f"[The Vail] crosses the river at {where}")

    for index, leg in enumerate(route.legs):
        if not leg.is_detour:
            continue
        print(f"[The Vail] leg {index} runs {leg.detour:.1f}x the crow's line — "
...
def main() -> None:
    ...
    print(f"[The Vail] {args.chapter}-{args.level} (seed {seed}): "
    ...
    if args.view in ("plan", "both"):
        ...
        if args.eagle:
            ...
            print(f"[The Vail] eagle: {flight.seconds:.0f} s aloft, {covered:.0f}% of the map, "
    ...
    if args.view in ("play", "both"):
        ...
        print(f"[The Vail] wrote {path} — {caravan.progress:.0%} along the {args.route} route, "
