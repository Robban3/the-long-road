...
capture-metoder — `TheVail.Editor.TheVailSetup.CaptureLevelPreview` och `CapturePlayScene` —
kräver en installation med GPU. Generatorn kräver ingenting av det: en bana är ett
recept plus ett frö, och den aritmetiken finns nu även i `Tools/vail_level.py`.
...
ur `TheVailSetup.cs` och `TerrainGround.shader`.
...
Djuren i GDD §3.5 ritas i spelvyn. Placeringen kommer ur porten av `TheVail.Sim.Wildlife`,
